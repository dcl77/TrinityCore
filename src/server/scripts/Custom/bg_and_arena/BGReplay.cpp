...
namespace
{
    Player* CreateReplayBot(Battleground* bg)
    {
        ...
        WorldSession* botSession = new WorldSession(0, "ReplayBot", std::shared_ptr<WorldSocket>(), SEC_ADMINISTRATOR, false, 2, 0, Minutes(0), LOCALE_enUS, 0, false);
        botSession->m_timeOutTime = std::numeric_limits<time_t>::max();
...
public:
    ...
    void OnPacketSend(WorldSession* session, WorldPacket& packet) override {
        ...
        if (needCreate)
        {
            Player* bot = CreateReplayBot(bg);
            std::lock_guard<std::mutex> lock(replayMutex);
            replayBots[instanceId] = bot;
        ...
        {
            record.startTime = GameTime::GetGameTimeMS() - bg->GetElapsedTime();
...
public:
    ...
    void OnBattlegroundUpdate(Battleground* bg, uint32 /*diff*/) override {
        ...
        if (startDelayTime > 50000)
        {
            ...
            bg->SetElapsedTime(bg->GetElapsedTime() + (startDelayTime - 50000));
        }
        ...
        else
        {
            ...
            while (!match.packets.empty() && match.packets.front().timestamp <= bg->GetElapsedTime()) {
                if (!bg->HaveSpectators())
                    break;
    ...
    void saveReplay(Battleground* bg) {
        ...
        for (auto it : match.packets)
        {
            ...
            buffer << uint16(it.packet.GetOpcode()); // 2 bytes explicitly written as uint16
            if (headerSize > 0)
...
public:
    ...
    struct replayAI : public ScriptedAI
    {
        ...
        bool StartReplay(Player* player, uint32 replayId) {
            ...
            if (loadedReplays[player->GetGUID().GetCounter()].packets.empty())
            {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_IN_PROGRESS, 0, bg->GetElapsedTime(), bg->GetArenaType(), teamId);
        ...
        bool loadReplayDataForPlayer(Player* p, uint32 matchId) {
            ...
            TC_LOG_INFO("bg.replay", "Loaded replay {} packets {} for spectator {}", matchId, loadedReplays[p->GetGUID().GetCounter()].packets.size(), p->GetGUID().ToString());
            return true;
        }
