...
uint16 WardenPayloadMgr::GetFreePayloadId()
{
    ...
    while (CachedChecks.find(payloadId) != CachedChecks.end())
    {
        ...
        {
            TC_LOG_ERROR("warden", "Max warden payload id of '{}' passed!", WardenPayloadMgr::WardenPayloadOffsetMax);
            return 0;
        }
    }
...
uint16 WardenPayloadMgr::RegisterPayload(const std::string& payload)
{
    ...
    {
        TC_LOG_ERROR("warden", "Failed to register payload.");
        return 0;
    }
...
bool WardenPayloadMgr::RegisterPayload(std::string const& payload, uint16 payloadId, bool replace)
{
    ...
    {
        TC_LOG_ERROR("warden", "Tried to register payloadId lower than '{}'.", WardenPayloadMgr::WardenPayloadOffsetMin);
        return false;
    }
    ...
    {
        TC_LOG_ERROR("warden", "Payload Id '{}' already exists in CachedChecks.", payloadId);
        return false;
    }

    WardenCheck wCheck;
    wCheck.Type = static_cast<WardenCheckType>(WardenPayloadMgr::WardenPayloadCheckType);
    wCheck.Str = payload;
    wCheck.CheckId = payloadId;

    std::string idStr = Trinity::StringFormat("%04u", payloadId);
    ASSERT(idStr.size() == 4);
...
void WardenPayloadMgr::QueuePayload(uint16 payloadId, bool pushToFront)
{
    ...
    if (it == CachedChecks.end())
    {
        TC_LOG_ERROR("warden", "Failed to queue payload id '{}' as it does not exist in CachedChecks.", payloadId);
        return;
    }
