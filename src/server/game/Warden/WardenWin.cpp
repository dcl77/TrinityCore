...
static uint16 GetCheckPacketSize(WardenCheck const* check)
{
    ...
    if (check->CheckId >= WardenPayloadMgr::WardenPayloadOffsetMin && check->Type == static_cast<WardenCheckType>(WardenPayloadMgr::WardenPayloadCheckType))
    {
...
void WardenWin::RequestChecks()
{
    ...
    // They are added first, so they are guaranteed to have space in the packet!
    std::vector<uint16> customChecks;
    while (!_payloadMgr.QueuedPayloads.empty())
    {
        ...
        TC_LOG_DEBUG("warden", "Adding custom warden payload '{}' to customChecks.", payloadId);
        _payloadMgr.QueuedPayloads.pop_front();
        customChecks.push_back(payloadId);
    ...
    // Erase normal checks that don't fit in the remaining packet space (expectedSize already includes customChecks!)
    ...
    // Merge customChecks into _currentChecks so they are processed in the outgoing packet
    _currentChecks.insert(_currentChecks.end(), customChecks.begin(), customChecks.end());
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        if (id >= WardenPayloadMgr::WardenPayloadOffsetMin && check->Type == static_cast<WardenCheckType>(WardenPayloadMgr::WardenPayloadCheckType))
        {
            buff << uint8(check->Str.size());
            buff.append(check->Str.data(), check->Str.size());
        }
        else if (check->Type == LUA_EVAL_CHECK || check->Type == static_cast<WardenCheckType>(WardenPayloadMgr::WardenPayloadCheckType))
        {
