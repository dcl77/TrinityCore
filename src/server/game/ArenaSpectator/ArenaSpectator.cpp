...
void ArenaSpectator::HandleResetCommand(Player* player)
{
    ...
    for (Battleground::BattlegroundPlayerMap::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
    {
        ...
        SendCommand_UInt32Value(player, itr->first, "TIM", (bg->GetElapsedTime() < 46 * MINUTE * IN_MILLISECONDS) ? (46 * MINUTE * IN_MILLISECONDS - bg->GetElapsedTime()) / IN_MILLISECONDS : 0);
...
bool ArenaSpectator::ShouldSendAura(Aura* aura, uint8 effMask, ObjectGuid targetGUID, bool remove)
{
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (effMask & (1 << i))
        {
            ...
            {
                AuraType at = effect->GetAuraType();
                if ((effect->GetAmount() && (aura->GetSpellInfo()->IsPositive() || targetGUID != aura->GetCasterGUID())) ||
